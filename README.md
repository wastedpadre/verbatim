...
CA installs by **pulling a public image** — it never builds from source. So the
image has to be published before a template can reference it.
...
**2. Let the workflow publish the image.** `.github/workflows/docker-publish.yml`
builds on every push to `main` and pushes to GHCR. After the first successful
...
**5. Submit at `ca.unraid.net/submit`.** It scans the repo, validates the
template XML and `ca_profile.xml`, checks for duplicate listings, and previews
the listing before anything is published.
